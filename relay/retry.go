...
func newRetryBuffer(name string, size, batch int, max time.Duration, p poster) *retryBuffer {
	r := &retryBuffer{
		...
		maxBuffered:     size,
		maxBatch:        batch,
		list:            newBufferList(name, size, batch),
		p:               p,
	}
	go r.run()
	return r
...
func newBufferList(name string, maxSize, maxBatch int) *bufferList {
	return &bufferList{
		name:     name,
